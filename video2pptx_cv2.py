...
audio_arr = ['mp3', 'wav']
video_arr = ['flv', 'avi', 'mp4', 'mov', 'rm', 'rmvb']
audio_file = None
exception_state = 0
...
def get_video_info(video_path):
    ...
    video_frames = vc.get(cv2.CAP_PROP_FRAME_COUNT)
    video_width = vc.get(cv2.CAP_PROP_FRAME_WIDTH)
    video_height = vc.get(cv2.CAP_PROP_FRAME_HEIGHT)

    # 获取视频文件时长
    if video_fps:
        video_duration = (int)((video_frames * 1.0) / video_fps)
    else:
        video_duration = 0

    print(video_size, video_fps, video_frames, video_width, video_height, video_duration)
    return vc
...
def video2pptx(process_queue, video_path, pptx_name, sim_threshold, output_path,
    ...
    vc = get_video_info(video_path)
    if video_frames < 1.0 or video_fps < 1.0 or video_width < 1.0 or video_height < 1.0:
...
if __name__ == "__main__":
    ...
    combo_stepframe['values'] = (100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 2000, 3000)
